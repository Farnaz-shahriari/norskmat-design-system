...
1. `src/app/utils/severityColors.ts`, Vesentlig container is wrong, currently `#ffddcc` / `#4a2800`, should be `#FDD19F` / `#3D2100`. Still needs to be applied to the live app.
2. Vesentlig main color, `#A84300`, is not wired into the app at all, one real component collapses Vesentlig and Mindre alvorlig into the same yellow icon color, the three tier system is not fully implemented in the UI yet. Still needs to be applied to the live app.
3. `src/styles/globals.css` has dead, unused variables, `--s-avvik-container`, `--l-avvik-container`, `--avvik-container`, none are referenced by any component, and their values do not match this file. Superseded by `--vesentlig-container` and `--mindre-alvorlig-container` in `assets/tokens.css`.
4. `globals.css` brand and surface tokens have drifted from this file in small ways for KSL, Nyt Norge, Spesialitet, and more substantially for LokalMat. **Resolved in `assets/tokens.css`**, which now ships the complete, correct value for every platform, use that file directly rather than the app's copy.
5. `inline-editable-list-item.tsx` used an orphan orange, confirmed a mistake, replaced with Vesentlig, `#A84300` main, `#FDD19F` / `#3D2100` container. No other orange belongs anywhere in the system.
6. **`--outline` is missing entirely from the app**, every platform, not just one. `switch.tsx` and the text input family hardcoded the raw hex directly because there was no variable to reach for. **Fixed in `assets/tokens.css`**, and in the corrected component copies in `assets/components/`.
7. **`--outline-variant` is missing from the app for Nyt Norge, Spesialitet, and LokalMat**, KSL is the only platform that had it. **Fixed in `assets/tokens.css`.**
8. **The app's `@theme inline` mapping is incomplete**, `outline`, `outline-variant`, and the entire surface family, `surface`, `surface-variant`, `on-surface`, `on-surface-variant`, and so on, were never exposed as Tailwind utility classes, even though the raw CSS variables existed. Any component using a class like `bg-surface-variant` or `border-outline-variant` silently does nothing in the live app today. **Fixed in `assets/tokens.css`**, which includes the complete mapping.
9. Messina Serif and Sands, LokalMat's fonts, are not licensed or loaded anywhere, not in this repo, not in the app. `assets/tokens.css` falls back to Georgia in the meantime so headings do not silently drop to a generic system sans, replace `--font-family` in the `.theme-lokalmat` block the moment the real font files exist. This one cannot be fixed from here, it needs the actual licensed font files.
